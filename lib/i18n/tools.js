...
const STRINGS = {
  ...
  "convert-image": {
    ...
    // No longer "pick the format first": the conversion re-runs when the
    // format changes, so there is nothing to decide before starting.
    dropManyHint: {
      nl: "meerdere tegelijk mag — het formaat kies je hierna",
      en: "several at once is fine — you pick the format afterwards",
    },
  ...
  "merge-pdf": {
    ...
    takePages: { nl: "Pagina's hieruit", en: "Pages from this one", ar: "الصفحات من هذا" },
    allOfIt: { nl: "allemaal", en: "all of it", ar: "كلها" },
    showPages: { nl: "Pagina's tonen", en: "Show the pages", ar: "إظهار الصفحات" },
    hidePages: { nl: "Verbergen", en: "Hide", ar: "إخفاء" },
    takeAll: { nl: "Weer allemaal", en: "Take all again", ar: "خذها كلها ثانية" },
    takingSome: { nl: "{n} van {total} pagina's", en: "{n} of {total} pages", ar: "{n} من {total} صفحة" },
    willTake: {
      nl: "{n} pagina's in totaal. Wat je niet kiest, gaat er niet in — de originele bestanden blijven zoals ze zijn.",
      en: "{n} pages in total. What you leave out stays out — your original files are untouched.",
      ar: "{n} صفحة في المجموع. ما لا تختاره لا يدخل — وملفاتك الأصلية تبقى كما هي.",
    },
    emptyPick: {
      nl: "Uit één van de bestanden wordt niets meegenomen. Bedoel je dat, of staat er een pagina in die er niet is?",
      en: "One of the files contributes nothing. Is that meant, or does the range name a page it does not have?",
      ar: "أحد الملفات لا يساهم بشيء. هل هذا مقصود، أم أن النطاق يذكر صفحة غير موجودة فيه؟",
    },
    nothingPicked: {
      nl: "Er is geen enkele pagina gekozen.",
      en: "No pages have been picked at all.",
      ar: "لم تُختَر أي صفحة إطلاقاً.",
    },
  ...
  // --------------------------------------------------------- arabic-text
  "arabic-text": {
    where: { nl: "Waar plak je dit?", en: "Where are you pasting this?", ar: "أين ستلصق هذا النص؟" },
    otherApp: { nl: "Iets anders", en: "Something else", ar: "برنامج آخر" },
    "note.capcut": {
      nl: "CapCut heeft op geen enkel platform een Arabische tekstmotor. Beide aan.",
      en: "CapCut has no Arabic text engine on any platform. Both switches on.",
      ar: "كاب كات بلا محرّك نصّي عربي على أي منصة. فعّل المفتاحين.",
    },
    "note.photoshop": {
      nl: "Alleen nodig in de standaardversie. Heeft die van jou de Midden-Oostenmotor, zet beide uit.",
      en: "Only needed on the standard build. If yours has the Middle Eastern text engine, turn both off.",
      ar: "مطلوب في النسخة القياسية فقط. إن كانت نسختك تحتوي محرّك نصوص الشرق الأوسط فأوقف المفتاحين.",
    },
    "note.aftereffects": {
      nl: "Zelfde regel als Photoshop — het hangt ervan af of de ME-tekstmotor erin zit.",
      en: "Same rule as Photoshop — it depends whether the ME text engine is installed.",
      ar: "نفس قاعدة فوتوشوب — يعتمد على وجود محرّك نصوص الشرق الأوسط.",
    },
    "note.premiere": {
      nl: "Het ondertitelpaneel maakt de letters los, ook in de huidige versies.",
      en: "The captions panel detaches the letters, even on current builds.",
      ar: "لوحة الترجمة تفصل الحروف، حتى في الإصدارات الحالية.",
    },
    "note.resolve": {
      nl: "Zowel Fusion als de Text+ node tekent Arabisch zonder verbindingen.",
      en: "Fusion and the Text+ node both draw Arabic unconnected.",
      ar: "كلٌّ من Fusion وعقدة Text+ يرسمان العربية بلا وصل.",
    },
    "note.blender": {
      nl: "Het tekstobject van Blender kent complexe schriften helemaal niet.",
      en: "Blender's text object has no complex-script support at all.",
      ar: "كائن النص في بليندر بلا دعم للخطوط المركّبة إطلاقاً.",
    },
    "note.engine": {
      nl: "De meeste game-engines zetten rechts-naar-links zelf goed neer zodra de letters al verbonden binnenkomen.",
      en: "Most game engines lay out right-to-left correctly once the letters arrive already joined.",
      ar: "أغلب محركات الألعاب ترتّب من اليمين لليسار بشكل صحيح متى وصلتها الحروف موصولة أصلاً.",
    },
    "note.other": {
      nl: "Begin met beide aan. Komt de tekst gespiegeld eruit, zet het omkeren uit.",
      en: "Start with both on. If the text comes out mirrored, turn the flip off.",
      ar: "ابدأ بالمفتاحين مفعّلين. إن ظهر النص معكوساً فأوقف العكس.",
    },
    yourText: { nl: "Jouw tekst", en: "Your text", ar: "نصّك" },
    placeholder: {
      nl: "Plak of typ hier Arabisch…",
      en: "Paste or type Arabic here…",
      ar: "الصق أو اكتب النص العربي هنا…",
    },
    fixed: { nl: "Gemaakt — kopieer dit", en: "Fixed — copy this", ar: "النص المصلَح — انسخ هذا" },
    outHint: {
      nl: "Met opzet van links naar rechts getoond: dit is precies de tekenvolgorde die je programma binnenkrijgt.",
      en: "Shown left to right on purpose — this is exactly the character order your editor receives.",
      ar: "معروض من اليسار إلى اليمين عمداً — هذا هو ترتيب الأحرف الذي سيستلمه برنامجك بالضبط.",
    },
    restore: { nl: "Origineel terughalen", en: "Restore the original", ar: "استعادة الأصل" },
    "state.clean": { nl: "Schoon Arabisch", en: "Clean Arabic", ar: "عربي سليم" },
    "state.processed": {
      nl: "Deze tekst is al door de tool geweest. Nog een keer is veilig — het origineel wordt eerst hersteld.",
      en: "This text has already been through the tool. Doing it again is safe — the original is recovered first.",
      ar: "هذا النص مرّ بالأداة من قبل. إعادته آمنة لأن الأصل يُستعاد أولاً.",
    },
    "state.none": { nl: "Geen Arabisch gevonden", en: "No Arabic found", ar: "لا يوجد نص عربي" },
    difference: { nl: "Zie het verschil", en: "See the difference", ar: "شاهد الفرق" },
    nowShows: { nl: "Wat je programma nu toont", en: "What your editor shows now", ar: "ما يعرضه برنامجك الآن" },
    thenShows: { nl: "Wat het daarna toont", en: "What it shows after", ar: "ما سيعرضه بعد الإصلاح" },
    demoHint: {
      nl: "In beide vakken staat het verbinden en herordenen van de browser uit, zodat ze tonen wat een programma zonder Arabische ondersteuning zou tekenen.",
      en: "Both boxes have the browser's own joining and reordering switched off, so they show what an editor with no Arabic support would draw.",
      ar: "كلا الصندوقين مُعطَّل فيهما وصل الحروف وإعادة الترتيب في المتصفح، ليعرضا ما سيرسمه برنامج بلا دعم للعربية.",
    },
    tune: { nl: "Fijn afstellen", en: "Fine-tune", ar: "ضبط دقيق" },
    join: { nl: "Letters verbinden", en: "Join the letters", ar: "وصل الحروف" },
    joinHint: {
      nl: "Geeft elke letter zijn juiste begin-, midden- of eindvorm.",
      en: "Gives every letter its correct initial, middle or final shape.",
      ar: "إعطاء كل حرف شكله الابتدائي أو الوسطي أو النهائي الصحيح.",
    },
    flip: { nl: "Richting omkeren", en: "Flip the direction", ar: "عكس الاتجاه" },
    flipHint: {
      nl: "Uit als het programma zelf al van rechts naar links zet.",
      en: "Off if the app already lays out right to left.",
      ar: "أوقفه إذا كان البرنامج يرتّب من اليمين لليسار أصلاً.",
    },
    tashkeel: { nl: "Klinkertekens houden", en: "Keep the diacritics", ar: "إبقاء التشكيل" },
    tashkeelHint: {
      nl: "Sommige lettertypes zetten ze na de bewerking verkeerd neer.",
      en: "Some fonts place them badly after shaping.",
      ar: "بعض الخطوط تضعها في مواضع خاطئة بعد المعالجة.",
    },
    normalise: { nl: "Letters gelijktrekken", en: "Normalise the letters", ar: "توحيد الحروف" },
    normaliseHint: {
      nl: "أ إ آ worden ا en ى wordt ي. Goed om te zoeken, fout om te publiceren.",
      en: "أ إ آ collapse to ا and ى to ي. Right for search, wrong for publishing.",
      ar: "يوحّد أ إ آ إلى ا و ى إلى ي. صحيح للبحث، خاطئ للنشر.",
    },
    digits: { nl: "Cijfers", en: "Digits", ar: "الأرقام" },
    digitsKeep: { nl: "Zoals getypt", en: "As typed", ar: "كما كُتبت" },
    displayOnly: {
      nl: "Goed om te weten: de gemaakte tekst is alleen om te tonen. Het is geen gewoon Arabisch meer, dus er valt niet in te zoeken, te spellingscontroleren of letter voor letter in te werken. Bewaar je origineel — of breng het resultaat hier terug en haal het origineel eruit.",
      en: "Worth knowing: the fixed text is display-only. It is no longer ordinary Arabic, so it cannot be searched, spell-checked or edited letter by letter. Keep your original — or bring the result back here and restore it.",
      ar: "من المفيد معرفته: النص المصلَح للعرض فقط. لم يعد عربية عادية، فلا يمكن البحث فيه ولا تدقيقه إملائياً ولا تعديله حرفاً حرفاً. احتفظ بالأصل — أو أعد النتيجة إلى هنا واستعدها.",
    },
  },
