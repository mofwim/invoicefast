 ...
 *
 * What that can mean depends on where it came from. Something typed here is
 * simply removed. Something that arrived from a calendar cannot really be
 * deleted — the next sync would bring it straight back — so it is remembered as
 * deleted instead, which keeps it gone *and* keeps it restorable.
 *
 * @returns {{state: object, permanent: boolean}}
 */
export function deleteAppointment(state, appointment) {
  const source = state.sources.find((s) => s.id === appointment.source?.id);

  if (source && source.kind === "manual") {
    const list = (state.items[source.id] || []).filter((event) => {
      const normalized = normalizeAppointment(event, {
        sourceId: source.id,
        sourceKind: source.kind,
      });
      return !normalized || normalized.dedupeKey !== appointment.dedupeKey;
    });

    return {
      state: {
        ...state,
        sources: state.sources.map((s) => (s.id === source.id ? { ...s, count: list.length } : s)),
        items: { ...state.items, [source.id]: list },
      },
      permanent: true,
    };
  }
  ...
  return {
    state: setOverride(state, appointment.dedupeKey, {
      hidden: true,
      deletedTitle: appointment.title,
      deletedStart: appointment.start,
      deletedAt: Date.now(),
    }),
    permanent: false,
  };
...
/** Everything hidden from a synced source, newest deletion first. */
export function listDeleted(state) {
  return Object.entries(state.overrides || {})
    .filter(([, override]) => override && override.hidden)
    .map(([key, override]) => ({
      key,
      title: override.deletedTitle || "Afspraak",
      start: override.deletedStart || 0,
      deletedAt: override.deletedAt || 0,
    }))
    .sort((a, b) => b.deletedAt - a.deletedAt);
}

const RESTORE_PATCH = { hidden: null, deletedTitle: null, deletedStart: null, deletedAt: null };

export function restoreAppointment(state, dedupeKey) {
  return setOverride(state, dedupeKey, RESTORE_PATCH);
}

export function restoreAllDeleted(state) {
  return listDeleted(state).reduce((current, entry) => restoreAppointment(current, entry.key), state);
}
...
export function buildTimeline(state) {
  ...
  const visible = dedupeAppointments(all)
    .map((appointment) => {
