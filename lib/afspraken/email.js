...
export function appointmentsFromEmail(raw, options = {}) {
  ...
  for (const candidate of candidates) {
    results.push({
      ...
      attachments,
      bring: candidate.bring,
      phone: candidate.phone,
      // A mail announcing that something is off still belongs in the list —
      // struck through, so the reader sees it is handled.
      status: candidate.cancelled ? "CANCELLED" : "CONFIRMED",
      emailSubject: mail.subject,
