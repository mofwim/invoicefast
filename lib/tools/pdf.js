...
export async function readDocument(file, { forEditing = true } = {}) {
  ...
  try {
    ...
    throw new Error(niceError(file.name, err));
  }
}
...
function niceError(name, err) {
  ...
  if (/encrypt/i.test(message)) {
    return `${name} is beveiligd met een wachtwoord en kan niet worden geopend.`;
  }
  if (/password/i.test(message)) return `${name} vraagt om een wachtwoord.`;
  return `${name} kon niet worden gelezen — is het wel een PDF?`;
}
...
export async function mergeFiles(files, { onProgress } = {}) {
  ...
  if (!pages) throw new Error("Er zaten geen pagina's in deze bestanden.");
...
export async function rotateAndDrop(file, { rotate = {}, drop = [] } = {}) {
  ...
  if (!doc.getPageCount()) throw new Error("Zo blijft er geen enkele pagina over.");
  return doc;
}
...
export async function imagesToPdf(files, { pageSize = "a4", margin = 36, background = "#ffffff" } = {}) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`${file.name} kan niet in een PDF — alleen JPG en PNG werken.`);
      }
    }
  ...
  if (!doc.getPageCount()) throw new Error("Geen afbeeldingen om om te zetten.");
  return doc;
}
