 ...
 * form is what turns "assemble a document out of parts of others" from four
 * round trips — split, download, upload, merge — into one, which is the shape
 * that job actually has in somebody's head.
 *
 * @param {(File|{file: File, pages?: number[]})[]} entries
 */
export async function mergeFiles(entries, { onProgress } = {}) {
  const { PDFDocument } = await loadPdfLib();
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    // No selection means the whole document. An empty one means the reader
    // deliberately took nothing from this file, which is not the same thing.
    const wanted = entry?.pages
      ? entry.pages.filter((index) => index >= 0 && index < source.getPageCount())
 ...
 * still has to be labelled "3 / 12" rather than "1 / 1", or the preview would
 * be showing something the finished document never does.
 */
export async function stampDocument(
  file,
  { text = "", opacity = 0.25, size = 48, angle = 45, colour = "#ff0000", numbers = false, firstNumber = 1, total = 0 } = {}
) {
 ...
 *
 * @param {File} file
 * @param {number} index which page, zero-based
 * @param {(sample: File) => Promise<any>} operate the same work, on one page
 * @returns {Promise<Uint8Array>} the finished sample, ready to render
 */
export async function samplePage(file, index, operate) {
  const one = await extractPages(file, [index]);
  const bytes = await one.save();
  const sample = new File([bytes], file.name, { type: "application/pdf" });
